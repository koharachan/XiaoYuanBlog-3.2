...
> 作为评估的一部分，为避免"只报问题、不解决"，我们同时对当前环境下可收敛的风险项做了按基线加固，再从攻击者视角复测。这样既能核验问题是否可修、也能确认加固到哪一层为止，剩余的部分留给选型与产品方共同评估。本文仅记录过程与现象，不下结论。

### 7.1 已执行的加固（均可复现、可回滚）

| 项 | 处理 | 复测结果 |
|---|---|---|
| PolarDB 超级用户 `admin` 默认口令 | 改用随机强口令 | 旧 `admin/postgres` 已拒绝；新口令正常 |
| 代理后端账号 `aurora` 默认口令 | 改用随机强口令 | 旧 `aurora/aurora` 已拒绝；新口令正常 |
| MaxScale 管理口 12371 | 由 `0.0.0.0` 收紧为仅本机 | 仅 `127.0.0.1` 监听 |
| pg_hba | 仅保留回环 `127.0.0.1/32`，移除 `0.0.0.0/0` | 非回环来源将被鉴权拒绝 |
| 网络层 | 对数据库/代理/备份/节点驱动等端口，仅回环放行、其余来源 DROP | 外网来源被拦截（SSH、网站端口未受影响） |

### 7.2 复测后的观察（加固覆盖不到的部分）

对外部、默认口令这两类初始风险，加固后均以"拒绝"收场，说明该类问题在交付前是可以收敛的。但复测中仍观察到以下现象，恰好落在配置加固无法触及的层面：

1. **本地 socket 认证为 `trust`**：在 unix socket 上无需口令即可连接，且可直接以 `admin` 身份取得超级用户。若攻击者已通过同机 Web 应用或其它本地入口获得一个进程级立足点，则可据此无口令取得数据库超级用户——这是一条配置层改动很难单独堵住的提权路径。该默认是否按生产基线收敛，建议结合同机应用面评估。

2. **代理后端账号与高权目录读取存在耦合**：将 `aurora` 撤销超级用户并授予对 `pg_authid` 的显式 SELECT 后（直接查询已可读，多库均已授权），代理启动时加载用户列表仍被拒绝、服务无法启动——即"代理可用"与"账号需具备读取用户目录权限"之间存在权衡。其设计取舍与演进方式，建议向产品方核实。

3. **内核基线与安全修复节奏**（见上文第六节）：配置加固无法覆盖到内核层面对上游修复的跟进程度；这一层面是否纳入交付承诺，属于采购阶段应明确的事项。

### 7.3 一说明

以上加固为评估用途，具体生产基线以贵方与产品方商定为准；加固亦可能带来运维习惯的变化（例如口令管理、连接来源收敛），这些属可预期、可管理的调整。本文仅如实记录过程，不作结论。
