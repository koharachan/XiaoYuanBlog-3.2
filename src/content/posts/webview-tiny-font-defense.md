---
title: "小字攻防：华为/vivo 浏览器强制放大超小字号的排查与解法"
published: 2026-08-17
updated: 2026-08-17
draft: false
description: "Mate 60 与 vivo Y33s 上卡片 5px 小字被强制放大、Firefox 却正常的排查实录：三次试错，最终用「放大渲染再整体缩回」收场"
image: ""
tags: ["WebView", "CSS", "移动端", "华为", "vivo", "踩坑"]
category: 教程
comment: true
---
...
> 💡 **问题一句话**：华为 Mate 60 与 vivo Y33s 上，邀请页卡片里 5px 的小字被强制放大；其他手机、所有浏览器、**连 Firefox 都一切正常**。
...
- 出问题的只有两个机型：华为 Mate 60（鸿蒙自带浏览器 / ArkWeb）和 vivo Y33s（OriginOS 自带浏览器）；
- 同一台 Mate 60 上装 **Firefox 打开，显示完全正常**——排除了「系统字体调大了」的可能；
- 其他手机和所有桌面浏览器也都正常。

三条线索指向同一个结论：不是页面代码的问题，是这两个**安卓自带浏览内核**对「过小字号」的处理与标准浏览器不同。剩下的问题只有一个：它们到底在什么时候、怎样放大了字号，以及怎么绕过去。
...
> ❌ **无效**：这个解法只能抑制部分内核的经典 font boosting，对华为 ArkWeb / vivo 浏览器的「最小字号」机制不起作用。上线后问题依旧。
